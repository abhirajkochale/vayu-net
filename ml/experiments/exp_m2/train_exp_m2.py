"""
VAYU-NET: EXP-M2 Multi-Task Model Training Pipeline.
Trains M2A (GridSat-only), M2B (IMERG-only), and M2C (Adaptive Modality Fusion)
...
from ml.experiments.exp_m2.losses import MaskedMultiTaskLoss, encode_coordinates, normalize_wind
...
LOGS_DIR = REPO_ROOT / "ml/experiments/exp_m2/logs"
...
def evaluate_split(model: ExpM2Model,
                   loader: DataLoader,
                   criterion: MaskedMultiTaskLoss,
                   device: torch.device) -> Tuple[float, Dict[str, float]]:
    """Evaluates validation loss, task-specific component metrics, and modality gates."""
    model.eval()
    total_loss = 0.0
    num_batches = 0
    loss_acc = {
    ...
    with torch.no_grad():
        for batch in loader:
            g_seq = batch["gridsat"].to(device) if model.mode in ["m2a_gridsat", "m2c_adaptive"] else None
            i_seq = batch["imerg"].to(device) if model.mode in ["m2b_imerg", "m2c_adaptive"] else None
            ...
            # Record gate alphas for M2C
            if "gate_alpha" in preds:
                alphas = preds["gate_alpha"].cpu().numpy()  # [B, 6, 1]
                all_gate_alphas.extend(alphas.flatten())
    ...
    if all_gate_alphas:
        metrics["val_gate_gridsat_mean"] = float(np.mean(all_gate_alphas))
        metrics["val_gate_gridsat_std"] = float(np.std(all_gate_alphas))
        metrics["val_gate_imerg_mean"] = float(1.0 - np.mean(all_gate_alphas))
...
def train_single_model(mode: str,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for batch in train_loader:
            optimizer.zero_grad()
            g_seq = batch["gridsat"].to(device) if mode in ["m2a_gridsat", "m2c_adaptive"] else None
            i_seq = batch["imerg"].to(device) if mode in ["m2b_imerg", "m2c_adaptive"] else None
        ...
        if "val_gate_gridsat_mean" in val_metrics:
            gate_str = f" | Gate(GS/IM): {val_metrics['val_gate_gridsat_mean']:.3f}/{val_metrics['val_gate_imerg_mean']:.3f}"
    ...
    # History saving
    hist_name = {
        "m2a_gridsat": "m2a_gridsat_only_history.json",
        "m2b_imerg": "m2b_imerg_only_history.json",
        "m2c_adaptive": "m2c_adaptive_fusion_history.json"
    }[mode]
...
def main():
    print("=" * 65)
    print("VAYU-NET: EXP-M2 MULTIMODAL ABLATION TRAINING")
    print("=" * 65)
    ...
    results = {}
    for mode in ["m2a_gridsat", "m2b_imerg", "m2c_adaptive"]:
    ...
    print("\n" + "=" * 65)
    print("VAYU-NET: EXP-M2 TRAINING SUMMARY (VALIDATION SELECTION)")
    print("=" * 65)
