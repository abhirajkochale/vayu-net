"""
VAYU-NET — DECOUPLED MULTI-SOURCE SATELLITE ARCHITECTURE EXPERIMENTS
=====================================================================
Executes the Decoupled Routing Experiment on the Expanded TRAIN Dataset:
  - CONTROL BASELINE: GridSat-only transfer representation
  - ARCH A: Decoupled Track (GridSat + TIR1/TIR2 to track; GridSat to center/intensity)
  - ARCH B: Decoupled Intensity (GridSat + TIR1/TIR2/WV to intensity/wind; GridSat to track/center)
  - ARCH C: Decoupled Full Model (GridSat + TIR1/TIR2 to center/track; GridSat + TIR1/TIR2/WV to intensity/wind)
  - ARCH CENTER: Decoupled Center (GridSat + TIR1/TIR2 to center; GridSat to track/intensity)
  - ABLATION I2: Decoupled Intensity Dual-IR (GridSat + TIR1/TIR2 to intensity; GridSat to track/center)

Evaluated strictly on the unchanged 298 TEST samples.
"""
...
class MultisourceDecoupledDataset(Dataset):
...
def evaluate_decoupled_loader(model: MultisourceDecoupledModel,
                              loader: DataLoader,
...
def train_decoupled_experiment(exp_name: str,
                               mode: str,
    ...
    set_seed(42)
    print(f"\n=======================================================")
    print(f"TRAINING DECOUPLED EXPERIMENT: {exp_name.upper()} (mode='{mode}')")
    print(f"=======================================================")
...
def plot_decoupled_curves(results: Dict[str, Any], output_dir: str = "docs/figures/multisource_decoupled"):
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    fig, axes = plt.subplots(2, 2, figsize=(15, 11), dpi=150)
    plt.subplots_adjust(hspace=0.3, wspace=0.25)

    palette = {
        "Control Baseline": "#7f7f7f",
        "Arch A: Decoupled Track": "#1f77b4",
        "Arch B: Decoupled Intensity": "#ff7f0e",
        "Arch C: Decoupled Full": "#2ca02c",
        "Arch Center: Decoupled Center": "#9467bd",
        "Ablation I2: Intensity (TIR1+2)": "#8c564b"
    }
    ...
    for exp_key, data in results.items():
        ...
        c = palette.get(exp_key, "#333333")
        ax.plot(epochs, [h["train_loss"] for h in hist], linestyle="--", alpha=0.35, color=c)
        ax.plot(epochs, [h["val_loss"] for h in hist], linestyle="-", lw=2, color=c, label=exp_key)
    ax.set_title("Total Loss (Train dotted vs. Val solid)", fontweight="bold")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ...
    for exp_key, data in results.items():
        ...
        epochs = [h["epoch"] for h in hist]
        c = palette.get(exp_key, "#333333")
        ax.plot(epochs, [h["val_center_dpe_km"] for h in hist], marker="o", markersize=4, color=c, label=exp_key)
    ...
    for exp_key, data in results.items():
        ...
        epochs = [h["epoch"] for h in hist]
        c = palette.get(exp_key, "#333333")
        ax.plot(epochs, [h["val_wind_mae_kt"] for h in hist], marker="s", markersize=4, color=c, label=exp_key)
    ...
    for exp_key, data in results.items():
        ...
        epochs = [h["epoch"] for h in hist]
        c = palette.get(exp_key, "#333333")
        ax.plot(epochs, [h["val_track_dpe_km"] for h in hist], marker="^", markersize=4, color=c, label=exp_key)
    ...
    out_file = Path(output_dir) / "decoupled_training_curves.png"
    plt.savefig(out_file, bbox_inches="tight")
    plt.close()
    print(f"Saved decoupled training curves to {out_file}")


def run_all_decoupled_experiments():
    print("=========================================================================")
    print("VAYU-NET — DECOUPLED MULTI-SOURCE SATELLITE ARCHITECTURE SUITE")
    print("=========================================================================")
    ...
    configs = [
        ("Control Baseline", "baseline_gridsat", "data/interim/ml/checkpoints/multisource_decoupled_baseline.pt"),
        ("Arch A: Decoupled Track", "arch_a_decoupled_track", "data/interim/ml/checkpoints/multisource_decoupled_track.pt"),
        ("Arch B: Decoupled Intensity", "arch_b_decoupled_intensity", "data/interim/ml/checkpoints/multisource_decoupled_intensity.pt"),
        ("Arch C: Decoupled Full", "arch_c_decoupled_full", "data/interim/ml/checkpoints/multisource_decoupled_full.pt"),
        ("Arch Center: Decoupled Center", "decoupled_center", "data/interim/ml/checkpoints/multisource_decoupled_center.pt"),
        ("Ablation I2: Intensity (TIR1+2)", "ablation_i2_decoupled_intensity_tir", "data/interim/ml/checkpoints/multisource_decoupled_intensity_tir.pt")
    ]

    for label, mode, ckpt in configs:
        res = train_decoupled_experiment(
            exp_name=label,
            mode=mode,
    ...
    prev_results_path = Path("data/interim/ml/multisource_transfer_expanded_results.json")
    prev_results = {}
    ...
    for label, mode, _ in configs:
        tm = results[label]["test_metrics"]
        vm = results[label]["val_metrics"]
        pc = results[label]["parameter_counts"]

        row = {
            "Experiment": label,
            "Mode": mode,
            "Trainable_Params": pc["trainable_parameters"],
            "Best_Val_Epoch": results[label]["best_epoch"],
            "Val_Loss": round(results[label]["val_loss"], 4),
            "Test_Loss": round(results[label]["test_loss"], 4),
    ...
    csv_out = "data/interim/ml/multisource_decoupled_comparison.csv"
    df_comp.to_csv(csv_out, index=False)
    print(f"\nSaved comparison CSV to {csv_out}")

    # Compile comprehensive results JSON
    results_out = {
        "metadata": {
            "title": "VAYU-NET Decoupled Multi-Source Architecture Experiment",
        ...
        "previous_late_fusion_results": {
            "EXP1_GridSat_Frozen": prev_results.get("experiments", {}).get("EXP-1", {}).get("test_metrics", {}),
            "EXP2_GridSat_TIR1_TIR2": prev_results.get("experiments", {}).get("EXP-2", {}).get("test_metrics", {}),
            "EXP3_GridSat_TIR1_TIR2_WV": prev_results.get("experiments", {}).get("EXP-3", {}).get("test_metrics", {})
        },
        "production_reference_benchmarks": {
            "Production_Center_CNN": {
                "center_mean_dpe_km": 106.3,
                "note": "Production CNN trained on full NIO archive"
            },
            "Production_Temporal_Track_GRU": {
                "track_aggregate_mean_dpe_km": 168.3,
                "note": "Production 2-layer GRU"
            },
            "Production_Phase_5B_Hybrid_Track": {
                "track_aggregate_mean_dpe_km": 142.1,
                "note": "Hybrid Kinematic-GRU production model"
            },
            "Production_Phase_6_Intensity": {
                "wind_mae_kt": 11.2,
                "note": "Phase 6 Intensity production model"
            }
        }
    }

    json_out = "data/interim/ml/multisource_decoupled_results.json"
    with open(json_out, "w") as f:
        json.dump(results_out, f, indent=2)
    print(f"Saved results JSON to {json_out}")

    # Plot curves
    plot_decoupled_curves(results)
    print("\nAll decoupled experiments complete.")


if __name__ == "__main__":
    run_all_decoupled_experiments()
