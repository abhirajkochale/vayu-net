...
# Configuration
RAW_DIR = "data/interim/gridsat_pilot/amphan_2020/raw"
OUT_DIR = "data/interim/gridsat_pilot/amphan_2020"
DOCS_DIR = "docs"
IMD_V2_CSV = "data/processed/imd_best_track_v2.csv"

STORM_ID = "NIO_2020_AMPHAN"
T0_STR = "2020-05-16T00:00:00+00:00"

# Exact 6 historical frames
FRAMES = [
    {"tag": "t-15h", "delta_h": -15, "ts": "2020-05-15T09:00:00+00:00", "file": "GRIDSAT-B1.2020.05.15.09.v02r01.nc"},
    {"tag": "t-12h", "delta_h": -12, "ts": "2020-05-15T12:00:00+00:00", "file": "GRIDSAT-B1.2020.05.15.12.v02r01.nc"},
    {"tag": "t-9h",  "delta_h": -9,  "ts": "2020-05-15T15:00:00+00:00", "file": "GRIDSAT-B1.2020.05.15.15.v02r01.nc"},
    {"tag": "t-6h",  "delta_h": -6,  "ts": "2020-05-15T18:00:00+00:00", "file": "GRIDSAT-B1.2020.05.15.18.v02r01.nc"},
    {"tag": "t-3h",  "delta_h": -3,  "ts": "2020-05-15T21:00:00+00:00", "file": "GRIDSAT-B1.2020.05.15.21.v02r01.nc"},
    {"tag": "t0",    "delta_h": 0,   "ts": "2020-05-16T00:00:00+00:00", "file": "GRIDSAT-B1.2020.05.16.00.v02r01.nc"},
]

# 3 Target references
TARGETS = [
    {"tag": "t+12h", "delta_h": 12, "ts": "2020-05-16T12:00:00+00:00", "file": "GRIDSAT-B1.2020.05.16.12.v02r01.nc"},
    {"tag": "t+24h", "delta_h": 24, "ts": "2020-05-17T00:00:00+00:00", "file": "GRIDSAT-B1.2020.05.17.00.v02r01.nc"},
    {"tag": "t+48h", "delta_h": 48, "ts": "2020-05-18T00:00:00+00:00", "file": "GRIDSAT-B1.2020.05.18.00.v02r01.nc"},
]
...
def process_frame(frame_info):
    ...
    # Save cropped NetCDF
    out_nc_path = os.path.join(OUT_DIR, f"gridsat_amphan_{frame_info['tag']}.nc")
    save_cropped_netcdf(out_nc_path, frame_info, cropped_lat, cropped_lon, decoded)
    
    # Save compressed npz
    out_npz_path = os.path.join(OUT_DIR, f"gridsat_amphan_{frame_info['tag']}.npz")
...
def save_cropped_netcdf(path, frame_info, lat, lon, data):
    with netCDF4.Dataset(path, 'w', format='NETCDF4') as nc:
        ...
        nc.title = f"VAYU-NET GridSat-B1 Pilot Frame - Cyclone AMPHAN ({frame_info['tag']})"
...
def generate_visual_qc(frames_data, imd_t0_fix):
    ...
    plt.suptitle("VAYU-NET GridSat-B1 Pilot Sequence — Cyclone AMPHAN (2020)\nFrozen 6-Frame Historical Input Sequence (t-15h to t0) Cropped to North Indian Ocean",
                 fontsize=14, fontweight='bold', y=0.98)
    
    qc_out1 = os.path.join(OUT_DIR, "amphan_qc_plot.png")
    qc_out2 = os.path.join(DOCS_DIR, "amphan_qc_plot.png")
...
def main():
    print("=" * 70)
    print("VAYU-NET GridSat-B1 Pilot Processing & Alignment Pipeline — AMPHAN (2020)")
    print("=" * 70)
    
    # Load IMD V2 dataset
    imd_df = pd.read_csv(IMD_V2_CSV)
    amphan_imd = imd_df[imd_df['storm_id'] == STORM_ID].copy()
    print(f"Loaded IMD Best Track V2 for {STORM_ID}: {len(amphan_imd)} rows.")
    
    # Check t0 and targets in IMD
    t0_row = amphan_imd[amphan_imd['timestamp_utc'] == T0_STR]
    if t0_row.empty:
        raise ValueError(f"IMD observation at t0 ({T0_STR}) not found!")
    imd_t0_fix = t0_row.iloc[0].to_dict()
    
    t12_row = amphan_imd[amphan_imd['timestamp_utc'] == TARGETS[0]['ts']]
    t24_row = amphan_imd[amphan_imd['timestamp_utc'] == TARGETS[1]['ts']]
    t48_row = amphan_imd[amphan_imd['timestamp_utc'] == TARGETS[2]['ts']]
    ...
    print("\nIMD Fixes Verified:")
    print(f"  t0   ({T0_STR}): Lat {imd_t0_fix['latitude']}°N, Lon {imd_t0_fix['longitude']}°E, Wind {imd_t0_fix['maximum_sustained_wind_kt']} kt, Pres {imd_t0_fix['central_pressure_hpa']} hPa, Cat: {imd_t0_fix['category']}")
    print(f"  +12h ({TARGETS[0]['ts']}): Lat {imd_t12_fix['latitude']}°N, Lon {imd_t12_fix['longitude']}°E, Wind {imd_t12_fix['maximum_sustained_wind_kt']} kt, Pres {imd_t12_fix['central_pressure_hpa']} hPa, Cat: {imd_t12_fix['category']}")
    print(f"  +24h ({TARGETS[1]['ts']}): Lat {imd_t24_fix['latitude']}°N, Lon {imd_t24_fix['longitude']}°E, Wind {imd_t24_fix['maximum_sustained_wind_kt']} kt, Pres {imd_t24_fix['central_pressure_hpa']} hPa, Cat: {imd_t24_fix['category']}")
    print(f"  +48h ({TARGETS[2]['ts']}): Lat {imd_t48_fix['latitude']}°N, Lon {imd_t48_fix['longitude']}°E, Wind {imd_t48_fix['maximum_sustained_wind_kt']} kt, Pres {imd_t48_fix['central_pressure_hpa']} hPa, Cat: {imd_t48_fix['category']}")
    ...
    frames_manifest_path = os.path.join(OUT_DIR, "amphan_frames_manifest.csv")
    ...
    sample_manifest_path = os.path.join(OUT_DIR, "amphan_sample_manifest.csv")
    sample_df.to_csv(sample_manifest_path, index=False)
    print(f"Saved sample-level manifest: {sample_manifest_path}")
    
    # Task 9: Automated Validation Checks A through K
    print("\nRunning Alignment Validation Checks A through K...")
    checks = {}
    ...
    # Task 8: Generate Visual QC
    print("\nGenerating Visual QC Plot...")
    generate_visual_qc(frames_data, imd_t0_fix)
    
    print("\n" + "=" * 70)
    print("AMPHAN PILOT PROCESSING COMPLETE!")
    print("=" * 70)
